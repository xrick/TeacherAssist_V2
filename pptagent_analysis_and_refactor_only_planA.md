...
This document provides a comprehensive analysis of PPTAgent v2's architecture and a detailed refactoring plan to replace TeacherAssist's deprecated Presenton engine.

**Approach**: **PPTAgent Core Only (Option A)** - Transplant PPTAgent's core modules directly into TeacherAssist backend with a modern React frontend. No MCP overhead, no DeepPresenter agent orchestration.

**Timeline**: 3-4 weeks

**Key Benefits**:
- Simple, focused implementation
- Direct LLM calls (Ollama/OpenAI)
...
## Part 0: Architecture Overview (Reference Only)

> **Note**: This section documents DeepPresenter's full architecture for reference. For TeacherAssist, we're only using **PPTAgent Core** (Part 1) without the agent orchestration layer.
...
### 2.7 Frontend Architecture & UI Design
...
#### 2.7.6 SSE Progress Integration
...
#### 2.7.8 Mobile Responsive Design
...
#### 2.7.9 Accessibility Requirements
...
#### 2.7.10 Animation & Micro-interactions
...
### 2.8 Frontend Implementation Roadmap
...
## Part 3: Implementation Roadmap (3-4 Weeks)
...
### Phase 4: Testing #### Phase 4: Testing & Deployment Deployment (Week 4)
...
### 4.1 Presenton vs PPTAgent Core

| Aspect | Presenton | PPTAgent Core |
|--------|-----------|---------------|
| Architecture | Black-box, external | Integrated Python library |
| Customization | Limited | Full control |
| Templates | Fixed | Extensible |
| Progress | None | Real-time SSE streaming |
| LLM Support | Ollama only | OpenAI-compatible (Ollama, OpenAI, etc.) |
| Debugging | Hard | Easy (native Python) |
| ARM64 | Emulation required | Native support |
| Dependencies | Heavy (~2GB) | Lightweight |
| Document Input | Limited | Markdown |
...
3. **Model compatibility**
   ...
   - Context budget limits may need tuning per model
   - Use OpenAI-compatible API normalization via LLMService

4. **Template quality**
   - Start with a few well-tested templates
   - Test layout induction with each template
   - Document template requirements (slide categories, layout variety)

### 4.3 Implementation Timeline

```
Week 1: Core Setup
├── Transplant pptagent/ core modules
├── Create LLMService for Ollama
├── Setup FastAPI backend structure
└── Basic template loading

Week 2: Generation Pipeline
├── Implement presentation_service.py
├── REST API endpoints
├── SSE progress streaming
└── Test with single template

Week 3: Frontend Development
├── Initialize React + Tailwind + shadcn/ui
├── Input panel (topic, content, file upload)
...
├── Remove Presenton from codebase
...
This refactoring replaces the opaque Presenton service with PPTAgent Core's well-documented, extensible architecture. The key benefits are:
...
## Appendix: File Mapping for Transplant

```
pptagent/                        →  backend/app/pptagent_core/
├── agent.py                     →  agent.py
├── apis.py                      →  apis.py
├── induct.py                    →  induct.py
├── llms.py                      →  llms.py
